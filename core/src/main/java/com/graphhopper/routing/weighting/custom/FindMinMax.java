...
public class FindMinMax {
    ...
    /**
     * This method throws an exception when this CustomModel would decrease the
     * edge weight compared to the specified baseModel as in such a case the
     * optimality of A* with landmarks can no longer be guaranteed (as the
     * preparation is based on baseModel).
     */
    ...
    /**
     * This method returns the smallest value possible in "min" and the smallest
     * value that cannot be exceeded by any edge in max.
     */
        ...
        if (first.condition().trim().equals("true")) {
            if (first.isBlock()) {
                for (List<Statement> subGroup : CustomModelParser.splitIntoGroup(first.doBlock())) {
                    findMinMaxForGroup(minMax, subGroup, lookup);
                }
                return;
            } else {
            ...
            for (Statement s : group) {
                ...
                if (s.isBlock()) {
                    ...
                    for (List<Statement> subGroup : CustomModelParser.splitIntoGroup(s.doBlock())) {
                        findMinMaxForGroup(tmp, subGroup, lookup);
                    }
                } else {
