...
public class OSMRestrictionConverter {
    ...
    /**
     * OSM restriction relations specify turn restrictions between OSM ways (of
     * course). This method rebuilds the topology of such a relation in the
     * graph representation, where the turn restrictions are specified in terms
     * of edge/node IDs instead of OSM IDs.
     *
     * @throws OSMRestrictionException if the given relation is either not valid
     * in some way and/or cannot be handled and shall be ignored
     */
    public static Triple<ReaderRelation, RestrictionTopology, RestrictionMembers> buildRestrictionTopologyForGraph(BaseGraph baseGraph, ReaderRelation relation, LongFunction<Iterator<IntCursor>> edgesByWay) throws OSMRestrictionException {
        ...
        if (restrictionMembers.isViaWay()) {
            if (containsDuplicateWays(restrictionMembers)) // For now let's ignore all via-way restrictions with duplicate from/to/via-members
    ...
    /**
     * Converts an OSM restriction to (multiple) single 'no' restrictions to be
     * fed into {@link RestrictionSetter}
     */
        ...
        for (int i = 2; i < edges.size(); i++) {
            ...
            while (iter.next()) {
                if (iter.getEdge() != edges.get(i)
                    ...
                    if (restriction.size() == 3 && restriction.get(0) == restriction.get(restriction.size() - 1)) // To prevent an exception in RestrictionSetter we need to prevent unambiguous
